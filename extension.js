...
export default class PowerColorExtension extends Extension {
        ...
        try {
            ...
            this._proxy.init_async(GLib.PRIORITY_DEFAULT, null, (proxy, result) => {
                try {
                    ...
                    this._proxySignalId = this._proxy.connect('g-properties-changed', () => this._sync());
                    this._settingsSignalId = this._settings.connect('changed', () => this._sync());
                    this._sync();
                } catch (e) {
    ...
    _sync() {
        ...
        const profile = this._proxy.get_cached_property('ActiveProfile')?.unpack() || 'balanced';
